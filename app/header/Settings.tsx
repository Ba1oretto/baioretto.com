...
import type { CloseButtonFunction } from "~/modal/Modal";
import Modal, { isClick, ModalInit } from "~/modal/Modal";
...
export function ToggleColorSchemeButton() {
  ...
  function onButtonClick() {
    fetcher.submit(null, { action: "action/update_theme", method: "post" });
  }
...
export function UploadFileButton() {
  const uid = useId();
  const input_id = uid + "uploader";

  const [ is_drag, setDrag ] = useState(false);
  ...
  function onModalOutsideClick() {
    modal.current.setOpen(false);
  }
  ...
  return (
    <Fragment>
      ...
      <Modal ref={ modal } className="fixed left-0 top-0 h-screen w-screen center bg-midnight-blue/80 z-40" onModalOutsideClick={ onModalOutsideClick }>
        <section className="relative p-16 rounded-3xl bg-abyss-blue">
          <label
            onDragOver={ dragover }
            ...
            htmlFor={ input_id }
            data-active={ is_drag }
            className="center file-uploader-panel"
          >
          ...
          <button
            ...
            className="absolute right-4 top-4 p-2 font-bold rounded-xl hover:bg-goldenrod-yellow hover:text-sienna-orange transition-all"
          >
            Close
          </button>
        </section>
