...
export function ToggleColorSchemeButton() {
  ...
  function onButtonClick() {
    fetcher.submit(null, { action: "/action/update_theme", method: "put" });
  }

  useEffect(() => {
    const state = fetcher.state !== "idle";
    document.documentElement.classList[state ? "add" : "remove"]("busying");
    setPending(state);
  }, [ fetcher ]);

  return (
    <button className={ classNames("circle-button", is_pending && "cursor-wait") } onClick={ onButtonClick } disabled={ is_pending }>
